...
let sortedLim = users.map(toNumber('Dolares')).sort(sort('Dolares'))
...
││ ${sortedLim.slice(0, len).map(({ jid, Dolares }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${corazones} 🤍*`).join`\n││ `}
│╰────────────────···
╰───────────═┅═──────────
