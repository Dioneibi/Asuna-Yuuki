...
let sortedLim = users.map(toNumber('corazones')).sort(sort('estrellas'))
...
││ ${sortedLim.slice(0, len).map(({ jid, estrellas }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${corazones} 🤍*`).join`\n││ `}
│╰────────────────···
╰───────────═┅═──────────
