...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let replace = {
      ...
      greeting, level, corazones, name, week, date, time, totalreg, rtotalreg, role,
      readmore: readMore
    }
    text = text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), (_, name) => '' + replace[name])

    const dbPath = './media/database/db.json'
    const dbContent = JSON.parse(await promises.readFile(dbPath))
    const category = "video"
    const random = Math.floor(Math.random() * dbContent.links[category].length)
    const videoGifLink = dbContent.links[category][random]

    const response = await fetch(videoGifLink)
    const gifBuffer = await response.buffer()

    await conn.react(m.chat, '🤍')
    await conn.reply(m.chat, '*ꪹ͜𓂃͡𝗖𝗮𝗿𝗴𝗮𝗻𝗱𝗼 𝗘𝗹 𝗠𝗲𝗻𝘂 𝗗𝗲 𝗹𝗮 𝗕𝗼𝘁...𓏲੭*', gifBuffer, { 
      contextInfo: { 
    ...
    await conn.sendFile(m.chat, gifBuffer, '', null, m, true, {
      mimetype: 'image/gif',
      filename: 'menu.gif',
      caption: text,
      contextInfo: {
    ...
    conn.reply(m.chat, 'Lo siento, ocurrió un error. Intenta de nuevo.', m)
    throw e
  }
}
